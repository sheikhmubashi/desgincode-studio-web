    ...
    <header className="w-full bg-white">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between h-16 lg:h-[150px] gap-10">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} className="nav-link cursor-pointer transition-colors duration-150 hover:text-[#C3D6D4]">
              {item.label}
            </Link>
          ))}
        </nav>
      ...
      {/* Mobile Navigation */}
      <nav
        className={`lg:hidden border-t border-gray-200 absolute w-full left-0 bg-white z-50 transition-all duration-300 ease-in-out overflow-hidden ${isMenuOpen
