...
export default async function ArchitecturePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 lg:px-12 pt-10 pb-20">
      ...
      {/* Title */}
      <h1 className="heading-page text-2xl lg:text-5xl mb-6">{data?.pageTitle ?? 'ARCHITECTURE'}</h1>
      ...
      {projects.length > 0 ? (
        <div className="flex flex-wrap items-end justify-between">
          {projects.map((project, index) => {
            ...
            if (index === 0) { // Residences
              width = 471;
              height = 249;
            } else if (index === 1) { // Buildings
              width = 396;
              height = 361;
            } else if (index === 2) { // Mixed-Use
              width = 302;
              height = 529;
            }
